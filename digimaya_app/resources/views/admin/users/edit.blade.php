<x-app-layout>
    <x-slot name="header">
        ...
        <div class="mt-2">
            <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Users', 'url' => route('admin.users.index')], ['label' => 'Edit']]" />
        </div>
    </x-slot>
                        ...
                        <div class="mb-4" x-show="role === 'advertiser'" x-cloak>
                            ...
                            <select id="parent_am_id" name="parent_am_id" class="mt-1 block w-full border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2">
                                ...
                                @foreach($accountManagers as $am)
                                    <option value="{{ $am->id }}" {{ old('parent_am_id', $user->parent_am_id) == $am->id ? 'selected' : '' }}>{{ $am->name }}</option>
                                @endforeach
                            </select>
