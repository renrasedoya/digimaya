<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Lead') }}: {{ $lead->contact_name }}
        </h2>
        <div class="mt-2">
            <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Leads', 'url' => route('admin.leads.index')], ['label' => 'Edit']]" />
        </div>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('admin.leads.update', $lead) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="business_name" name="business_name" type="text" class="mt-1 block w-full" :value="old('business_name', $lead->business_name)" />
                            <x-input-error :messages="$errors->get('business_name')" class="mt-2" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="website_url" value="Website URL" />
                            <x-text-input id="website_url" name="website_url" type="text" class="mt-1 block w-full" :value="old('website_url', $lead->website_url)" placeholder="https://example.com" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="contact_name" name="contact_name" type="text" class="mt-1 block w-full" :value="old('contact_name', $lead->contact_name)" required autofocus />
                            <x-input-error :messages="$errors->get('contact_name')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="contact_email" value="Contact Email" />
                            <x-text-input id="contact_email" name="contact_email" type="email" class="mt-1 block w-full" :value="old('contact_email', $lead->contact_email)" />
                            <x-input-error :messages="$errors->get('contact_email')" class="mt-2" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="contact_phone" value="WhatsApp" />
                            <x-text-input id="contact_phone" name="contact_phone" type="text" class="mt-1 block w-full" :value="old('contact_phone', $lead->contact_phone)" placeholder="08xxxxxxxxxx" />
                        ...
                        <div x-data="{
                                interest: '{{ old('interested_in', $lead->interested_in ?? '') }}',
                                otherText: '{{ old('interested_in_other', $lead->interested_in_other ?? '') }}',
                                init() {
                            ...
                            <div class="mb-4">
                                ...
                                <select id="interested_in" name="interested_in" x-model="interest" class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                                    ...
                                    @foreach (\App\Models\Lead::INTERESTED_IN_OPTIONS as $key => $label)
                                        <option value="{{ $key }}" {{ old('interested_in', $lead->interested_in) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="monthly_ad_budget" name="monthly_ad_budget" class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                                ...
                                @foreach (\App\Models\Lead::BUDGETS as $key => $label)
                                    <option value="{{ $key }}" {{ old('monthly_ad_budget', $lead->monthly_ad_budget) === $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <select id="assigned_to" name="assigned_to" class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                                ...
                                @foreach ($assignableUsers as $user)
                                    <option value="{{ $user->id }}" {{ (int) old('assigned_to', $lead->assigned_to) === $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="message" name="message" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2" placeholder="Initial inquiry message or internal notes...">{{ old('message', $lead->message) }}</textarea>
                            <x-input-error :messages="$errors->get('message')" class="mt-2" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="disqualification_reason" value="Disqualification Reason" />
                            <textarea id="disqualification_reason" name="disqualification_reason" rows="2" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2" placeholder="Only fill if status is Disqualified">{{ old('disqualification_reason', $lead->disqualification_reason) }}</textarea>
