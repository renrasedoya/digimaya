    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <div class="mt-2">
                    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Services']]" />
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                        ...
                        <nav class="-mb-px flex space-x-8">
                            @php
                                ...
                                $tabs = [
                                    ...
                                    'agency' => 'Agency (' . $counts['agency'] . ')',
                                    'academy' => 'Academy (' . $counts['academy'] . ')',
                                    'active' => 'Active (' . $counts['active'] . ')',
                                    'inactive' => 'Inactive (' . $counts['inactive'] . ')',
                                ];
                            @endphp
                            @foreach($tabs as $key => $label)
                                <a href="{{ route('admin.services.index', $key === 'all' ? [] : ['filter' => $key]) }}"
                    ...
                    {{-- Search --}}
                    <form method="GET" action="{{ route('admin.services.index') }}" class="mb-6 flex gap-2">
                                ...
                                @foreach($services as $service)
                                    <tr>
                                        ...
                                        <td class="px-3 py-2 text-gray-600">
                                            <span class="inline-flex px-2 py-1 text-xs rounded-full {{ $service->category === 'agency' ? 'bg-blue-100 text-blue-800' : ($service->category === 'academy' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800') }}">
                                        ...
                                        <td class="px-3 py-2 text-gray-600 text-sm">{{ $service->sort_order }}</td>
                                        <td class="px-3 py-2 text-right">
                                            <a href="{{ route('admin.services.edit', $service) }}" class="text-indigo-600 hover:text-indigo-900 text-sm">Edit</a>
