    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <div class="mt-2">
                    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Expense', 'url' => route('admin.expenses.index')], ['label' => 'Add Expense']]" />
                </div>
            </div>
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.expenses.store') }}" class="space-y-6">
                                ...
                                <div class="grid grid-cols-2 gap-2 mt-1">
                                    <select id="expense_month" name="expense_month" required
                                        ...
                                        @foreach(range(1, 12) as $m)
                                            <option value="{{ $m }}" {{ (int) old('expense_month', now()->month) === $m ? 'selected' : '' }}>{{ \Carbon\Carbon::create()->month($m)->format('F') }}</option>
                                        @endforeach
                                    </select>
                                    <select id="expense_year" name="expense_year" required
                                            class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 block w-full">
                                        @for($y = now()->year + 1; $y >= 2026; $y--)
                                            <option value="{{ $y }}" {{ (int) old('expense_year', now()->year) === $y ? 'selected' : '' }}>{{ $y }}</option>
                                        @endfor
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="vendor_name" name="vendor_name" value="{{ old('vendor_name') }}" maxlength="255"
                                       class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                                <p class="mt-1 text-xs text-gray-500">Vendor or supplier name (e.g. Google Workspace).</p>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="recurring_type" name="recurring_type" required
                                    ...
                                    @foreach(App\Models\Expense::RECURRING_TYPES as $key => $label)
                                        <option value="{{ $key }}" {{ old('recurring_type', 'one_time') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
