<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Proposal Snippets') }}</h2>
                <div class="mt-2">
                    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Proposal Snippets']]" />
                </div>
            </div>
            <a href="{{ route('admin.proposal-snippets.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                + Add Snippet
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="border-b border-gray-200 mb-6">
                        <nav class="-mb-px flex space-x-8">
                            ...
                            @foreach($tabs as $key => $label)
                                <a href="{{ route('admin.proposal-snippets.index', $key === 'all' ? [] : ['filter' => $key]) }}"
                    ...
                    <form method="GET" action="{{ route('admin.proposal-snippets.index') }}" class="mb-6 flex gap-2">
                    ...
                    @if($snippets->isEmpty())
                        <p class="text-gray-500 text-center py-8">No snippets found.</p>
                    @else
                        ...
                        <div class="mt-12">{{ $snippets->links() }}</div>
                    @endif
