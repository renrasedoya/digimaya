    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div>
                ...
                <div class="mt-2">
                    <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Income', 'url' => route('admin.incomes.index')], ['label' => 'Edit Income']]" />
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="p-6">
                    ...
                    <form method="POST" action="{{ route('admin.incomes.update', $income) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6"
                             x-data="{
                                category: '{{ old('source_category', $income->source_category) }}',
                                hasService: false,
                            ...
                            <div>
                                ...
                                <select id="service_id" name="service_id" x-on:change="syncFromService($event.target)"
                                    ...
                                    @foreach($services as $service)
                                        <option value="{{ $service->id }}" data-category="{{ $service->category }}" {{ old('service_id', $income->service_id) == $service->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="client_id" name="client_id" placeholder="Search client by name...">
                                <option value=""></option>
                                @if($income->client)
                                    <option value="{{ $income->client->id }}" selected>{{ $income->client->business_name }}</option>
                                @endif
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-currency-input name="amount" :value="old('amount', $income->amount)" required />
                            </div>

                            <div>
                                <label for="received_date" class="block text-sm font-medium text-gray-700">Received Date <span class="text-red-500">*</span></label>
                                <input type="date" id="received_date" name="received_date" value="{{ old('received_date', $income->received_date->format('Y-m-d')) }}" required
                                       class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="reference_number" name="reference_number" value="{{ old('reference_number', $income->reference_number) }}" maxlength="255"
                                       class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" maxlength="5000"
                                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2">{{ old('description', $income->description) }}</textarea>
                        </div>
