<x-app-layout>
    <x-slot name="header">
        ...
        <div class="mt-2">
            <x-breadcrumb :items="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Clients', 'url' => route('admin.clients.index')], ['label' => 'Add New']]" />
        </div>
    </x-slot>
                    ...
                    <form method="POST" action="{{ route('admin.clients.store') }}">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="address" name="address" rows="3" maxlength="1000"
                                      ...
                                      class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">{{ old('address') }}</textarea>
                        ...
                        <div x-data="{ clientStatus: '{{ old('status', 'prospect') }}', allowedTargets: @js(\App\Models\Client::getAllowedTargetsFrom(null)) }">
                        <div class="mb-4">
                            ...
                            <select id="status" name="status" x-model="clientStatus" class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full" required>
                                <option value="prospect" x-show="allowedTargets.includes('prospect')" {{ old('status', 'prospect') === 'prospect' ? 'selected' : '' }}>Prospect</option>
                                <option value="active" x-show="allowedTargets.includes('active')" {{ old('status') === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" x-show="allowedTargets.includes('inactive')" {{ old('status') === 'inactive' ? 'selected' : '' }}>Inactive</option>
                                <option value="churned" x-show="allowedTargets.includes('churned')" {{ old('status') === 'churned' ? 'selected' : '' }}>Churned</option>
                                <option value="lost" x-show="allowedTargets.includes('lost')" {{ old('status') === 'lost' ? 'selected' : '' }}>Lost</option>
                            </select>
                            ...
                            <div class="mb-4">
                                ...
                                <select id="interested_in" name="interested_in" x-model="interest" class="border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm px-3 py-2 mt-1 block w-full">
                                    ...
                                    @foreach (\App\Models\Client::INTERESTED_IN_OPTIONS as $key => $label)
                                        <option value="{{ $key }}" {{ old('interested_in') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="client_since" name="client_since" type="date" class="mt-1 block w-full" :value="old('client_since')" />
                            <x-input-error :messages="$errors->get('client_since')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="monthly_retainer" value="Monthly Retainer (Rp)" />
                            <x-currency-input name="monthly_retainer" :value="old('monthly_retainer')" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-currency-input name="acquisition_cost" :value="old('acquisition_cost')" />
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-3 py-2">{{ old('notes') }}</textarea>
